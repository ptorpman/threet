...
static uint32_t gPacketSize = 64;   /* Packet size */
...
main(int argc, char **argv)
{
    ...
    if (gIsServer)
    {
        debug_print ("Starting server for %u connections...\n", gNumConn);
...
//        signal(SIGPIPE, SIG_IGN);
        ...
        client_start(gServerIp, gServerPort, gNumConn, gWantedTp, gPacketSize,
                     procId);
    }
    return 0;
}
...
send_msg_to_aggregator(uint8_t * msg, int msgLen, char *server_ip)
{
     ...
     * Set reusable socket 
     */
    setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *) &reuse,
               sizeof (reuse));
